...
export function BookSearch({ onAddBook, onAddToWishlist }: BookSearchProps) {
  ...
  const handleSearch = async (append = false) => {
    ...
    const currentStartIndex = append ? startIndex : 0
    ...
    try {
      const response = await fetch(
        `/api/books/search?q=${encodeURIComponent(query)}&startIndex=${currentStartIndex}&maxResults=${maxResults}`
      )
  ...
  const hasMoreResults = results.length < totalItems
      ...
      {results.length > 0 && (
        <div className="space-y-4">
          <div className="text-sm text-muted-foreground">
            Showing {results.length} of {totalItems.toLocaleString()} results
          </div>
